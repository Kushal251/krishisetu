...
import { AlertTriangle, ArrowRightLeft, Boxes, CheckCircle2, CloudSun, ShoppingCart, Sprout, TrendingUp, Warehouse } from "lucide-react";
...
export default function CenterDashboardPage() {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/me").then((response) => response.json()).then((payload) => { if (!["CENTER", "ADMIN"].includes(payload.user?.role)) router.replace("/"); }).catch(() => router.replace("/"));
  ...
  const { center, kpis, pendingPurchases, listings, intelligence } = data;
  return <><LoggedInNavbar /><main className="min-h-screen bg-[#f6f8f4] p-5 sm:p-8"><div className="mx-auto max-w-6xl"><p className="text-xs font-bold tracking-wider text-green-700">CENTER OPERATIONS</p><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="mt-1 text-3xl font-black">{center.name}</h1><p className="mt-2 text-gray-600">Confirm seller purchases, manage storage, and trade soybean with other centers.</p></div><div className="flex flex-wrap gap-2"><Link href="/center/market" className="inline-flex items-center gap-2 rounded-xl bg-green-700 px-4 py-2.5 text-sm font-bold text-white"><ShoppingCart size={17} />Buy from centers</Link><Link href="/center/trades" className="inline-flex items-center gap-2 rounded-xl border border-green-200 bg-white px-4 py-2.5 text-sm font-bold text-green-800"><ArrowRightLeft size={17} />Trade requests & history</Link></div></div>{error && <p className="mt-5 rounded-xl bg-red-50 p-3 font-bold text-red-700">{error}</p>}{message && <p className="mt-5 rounded-xl bg-green-50 p-3 font-bold text-green-800">{message}</p>}
    ...
    {intelligence && <section className="mt-7 rounded-3xl bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black tracking-wider text-green-700">SEASON FORECAST</p><h2 className="mt-1 text-2xl font-black">Supply and local demand outlook</h2><p className="mt-1 max-w-3xl text-sm text-gray-600">Uses verified farmer declarations, current bookings and orders, previous season activity, center trades, and admin-entered static weather.</p></div><span className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-black ${intelligence.decision === "BUY" ? "bg-amber-100 text-amber-900" : intelligence.decision === "SELL" ? "bg-blue-100 text-blue-900" : "bg-green-100 text-green-900"}`}>{intelligence.decision === "BUY" && <AlertTriangle size={16} />}{intelligence.decision}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Insight icon={<Sprout size={20} />} label="Expected season supply" value={`${number(intelligence.expectedSeasonSupply)} qtl`} detail={`${number(intelligence.declaredIncoming)} qtl from declarations`} /><Insight icon={<TrendingUp size={20} />} label="Projected local demand" value={`${number(intelligence.projectedDemand)} qtl`} detail={`${number(intelligence.currentDemand)} qtl observed now`} /><Insight icon={<Boxes size={20} />} label="Season balance" value={`${number(intelligence.shortage || intelligence.surplus)} qtl`} detail={intelligence.shortage > 0 ? "Expected shortage" : "Expected surplus"} /><Insight icon={<CloudSun size={20} />} label="Static weather" value={intelligence.weather?.condition || "Normal"} detail={`${number(intelligence.weather?.rainfallMm)} mm · ${number(intelligence.weather?.yieldFactor || 1)}x yield`} /></div><div className="mt-5 rounded-2xl bg-[#f6f8f4] p-4"><p className="font-black text-gray-900">{intelligence.message}</p>{intelligence.decision === "BUY" && <Link href="/center/market" className="mt-3 inline-flex items-center gap-2 rounded-xl bg-green-700 px-4 py-2 text-sm font-bold text-white">Review ranked suppliers · suggested {number(intelligence.recommendedTradeQty)} qtl</Link>}<p className="mt-3 text-xs text-gray-500">Previous season: {number(intelligence.previousSeasonSupply)} qtl supply · {number(intelligence.previousSeasonDemand)} qtl demand · reliability {number(intelligence.reliabilityPct)}%</p></div></section>}
...
function Insight({ icon, label, value, detail }) { return <article className="rounded-2xl border border-gray-100 p-4"><div className="text-green-700">{icon}</div><p className="mt-3 text-xs font-bold uppercase text-gray-500">{label}</p><p className="mt-1 text-xl font-black">{value}</p><p className="mt-1 text-xs text-gray-500">{detail}</p></article>; }
