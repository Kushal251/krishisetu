...
export async function GET() {
  try {
    ...
    if (!user || !["SELLER", "ADMIN"].includes(user.role) || (user.role === "SELLER" && !user.seller))
      return NextResponse.json({ message: "Seller or admin access required." }, { status: 403 });
    await markMissedBookings();

    const bookings = await prisma.booking.findMany({
      where: user.role === "ADMIN" ? {} : { sellerId: user.seller.id },
      include: { seller: { select: { user: { select: { name: true, phone: true } } } }, center: { select: { name: true, district: true, state: true, address: true, phone: true } }, inspection: true },
...
export async function POST(request) {
  try {
    ...
    const { centerId, visitDate, slotStart, quantity, password, sellerId: requestedSellerId } = await request.json();
    const targetSeller = user.role === "ADMIN" ? await prisma.seller.findUnique({ where: { id: requestedSellerId || "" }, select: { id: true } }) : user.seller;
    if (!targetSeller) return NextResponse.json({ message: "Select a valid seller before creating an admin booking." }, { status: 400 });
    ...
    if (!centerId || (user.role !== "ADMIN" && !password) || Number.isNaN(start.valueOf()) || start < new Date(new Date().setUTCHours(0, 0, 0, 0)))
    ...
    if (user.role !== "ADMIN" && !(await comparePassword(password, user.password)))
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const existing = await tx.booking.findFirst({ where: { sellerId: targetSeller.id, centerId, status: { notIn: ["CANCELLED", "NO_SHOW"] }, slotStart: { lte: end }, slotEnd: { gte: start } }, select: { id: true } });
      ...
      const booking = await tx.booking.create({ data: { sellerId: targetSeller.id, centerId, crop: "SOYBEAN", quantity: numericQuantity, slotStart: start, slotEnd: end }, include: { center: { select: { name: true } } } });
