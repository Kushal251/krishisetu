...
export async function PATCH(request, { params }) {
  try {
    ...
    if (user?.role !== "SELLER" || !user.seller) return NextResponse.json({ message: "Seller access required." }, { status: 403 });
    const { bookingId } = await params;
    const { action } = await request.json();
    const booking = await prisma.booking.findFirst({ where: { id: bookingId, sellerId: user.seller.id }, include: { inspection: true, center: { select: { name: true } } } });
    ...
    if (action === "ACCEPT_OFFER") {
      await prisma.$transaction(async (tx) => {
        ...
        const operators = await tx.user.findMany({ where: { role: "CENTER", centerId: booking.centerId }, select: { id: true } });
