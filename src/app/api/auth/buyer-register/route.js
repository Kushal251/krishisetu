...
export async function POST(request) {
  try {
    ...
    const { name, email, password, buyerType, businessName, gstin, panNumber, address, state, division, district, village } = body;
    ...
    if (!name?.trim() || !email?.trim() || !password || !businessName?.trim() || !gstin?.trim() || !panNumber?.trim() || !address?.trim()) return NextResponse.json({ message: "Please complete every buyer and business field." }, { status: 400 });
    ...
    const user = await prisma.user.create({ data: { name: name.trim(), phone, email: email.trim().toLowerCase(), password: await hashPassword(password), role: "BUYER", buyer: { create: { buyerType, businessName: businessName.trim(), gstin: gstin.trim().toUpperCase(), panNumber: panNumber.trim().toUpperCase(), address: address.trim(), ...location, verificationRequests: { create: { note: "Automatically submitted when the buyer account was created." } } } } }, select: { id: true } });
