...
function StatusCard({ seller, buyer }) {
  const profile = buyer || seller;
  const verified = profile?.verificationStatus === "VERIFIED";
  const rejected = profile?.verificationStatus === "REJECTED";
  const latestRequest = profile?.verificationRequests?.[0];
  const kind = buyer ? "buyer" : "seller";
  return <section className={`rounded-2xl p-5 text-white ${verified ? "bg-green-800" : rejected ? "bg-red-700" : "bg-amber-700"}`}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-white/75">Profile status</p><h2 className="mt-2 text-2xl font-black">{verified ? `Verified ${kind}` : rejected ? "Verification rejected" : "Verification pending"}</h2><p className="mt-2 max-w-xl text-sm text-white/85">{verified ? `Your ${kind} profile is verified.` : rejected ? "Review the admin reason and submit a new verification request from your profile." : "Your verification request is with the admin."}</p>{rejected && latestRequest?.adminNote && <p className="mt-3 rounded-lg bg-white/15 p-3 text-sm"><span className="font-black">Admin reason:</span> {latestRequest.adminNote}</p>}</div>{verified ? <CheckCircle2 size={38} /> : <CircleAlert size={38} />}</div><Link href="/profile" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-bold text-gray-900">{rejected ? "View reason & reapply" : "View profile"} <ArrowRight size={16} /></Link></section>;
}

function BookingSummary({ bookings }) {
  if (!bookings.length) return <section className="rounded-2xl border border-dashed border-green-300 bg-green-50 p-5"><p className="text-xs font-bold tracking-wider text-green-700">PROCUREMENT SLOTS</p><h2 className="mt-1 text-xl font-black text-green-950">No booking yet</h2><p className="mt-2 text-sm text-green-900">Compare soybean prices and reserve your visit at a nearby center.</p><Link href="/centers" className="mt-4 inline-flex items-center gap-2 text-sm font-black text-green-800">Browse centers <ArrowRight size={16} /></Link></section>;
  const statusText = { BOOKED: "Slot confirmed — arrive during your selected dates.", ARRIVED: "Arrival recorded — testing will start shortly.", INSPECTION: "Quality testing is in progress.", GRADED: "Grade result and price offer are ready.", COMPLETED: "Sale completed — final bill is ready.", NO_SHOW: "Slot closed because arrival was missed.", CANCELLED: "Booking cancelled." };
  return <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-wider text-green-700">PROCUREMENT SLOTS</p><h2 className="mt-1 text-xl font-black text-gray-900">My bookings</h2></div><Link href="/bookings" className="text-sm font-bold text-green-700 hover:text-green-800">View all</Link></div><div className="mt-4 space-y-3">{bookings.slice(0, 3).map((booking) => <article key={booking.id} className="rounded-xl border border-gray-100 bg-gray-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-gray-900">{booking.center.name}</p><p className="mt-1 flex items-center gap-1 text-sm text-gray-600"><MapPin size={14} />{booking.center.district}, {booking.center.state}</p><p className="mt-2 text-sm font-bold text-green-800">Soybean · {Number(booking.quantity)} quintal</p><p className="mt-1 text-sm font-bold text-gray-700">{statusText[booking.status]}</p>{booking.inspection && <p className="mt-1 text-sm text-green-800">Grade {booking.inspection.grade} · Offer ₹{Number(booking.inspection.gradePrice).toLocaleString("en-IN")} / quintal</p>}</div><div className="rounded-lg bg-green-100 px-3 py-2 text-right text-xs font-bold text-green-900"><p className="flex items-center gap-1"><CalendarDays size={14} />{new Date(booking.slotStart).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</p><p className="mt-1">{booking.status.replace("_", " ")}</p></div></div></article>)}</div></section>;
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/seller/bookings", { cache: "no-store" })
      .then(async (response) => response.ok ? (await response.json()).bookings : [])
      .then(setBookings)
      .catch(() => {});
  }, []);
  ...
  return <><LoggedInNavbar user={user} /><main className="min-h-screen bg-[#f6f8f4]"><div className="mx-auto max-w-6xl p-5 sm:p-8"><section className="rounded-3xl bg-gradient-to-br from-green-950 to-green-700 px-6 py-8 text-white sm:px-9"><p className="text-sm font-bold text-lime-300">WELCOME BACK</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">Namaste, {user.name}</h1><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-green-100">{seller && <span className="inline-flex items-center gap-2"><MapPin size={16} />{seller.village}, {seller.district}</span>}<span className="inline-flex items-center gap-2"><BellRing size={16} />Government updates and schemes</span></div></section>
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]"><div className="space-y-6"><StatusCard seller={seller} buyer={buyer} />{user.role === "SELLER" && <BookingSummary bookings={bookings} />}{seller?.sellerType === "FARMER" && seller.verificationStatus === "VERIFIED" && <SeasonBanner />}<section><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold tracking-wider text-green-700">LATEST FOR FARMERS</p><h2 className="mt-1 text-2xl font-black">Forms &amp; updates</h2></div><span className="rounded-full bg-gray-200 px-3 py-1 text-xs font-bold text-gray-600">Static demo data</span></div><div className="mt-4 space-y-3">{updates.map((update) => { const Icon = update.icon; return <article key={update.title} className="flex gap-4 rounded-2xl bg-white p-5 shadow-sm"><div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${update.tone === "amber" ? "bg-amber-100 text-amber-800" : update.tone === "blue" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"}`}><Icon size={21} /></div><div><p className="text-xs font-bold text-gray-500">{update.type}</p><h3 className="mt-1 font-black">{update.title}</h3><p className="mt-1 text-sm text-gray-600">{update.text}</p></div></article>; })}</div></section></div>
